...
var spotifyApi = {
  ...
  async status() {
    return await jsonFetch(spotifyApiPath("status"));
  },
  ...
  async accessToken() {
    return await jsonFetch(spotifyApiPath("accesstoken"));
  },
  ...
  async search(q, type = "track,artist,album,playlist", limit = 10, offset = 0) {
    ...
    return await cachedGet(`spotify:${url}`, url);
  },
...
var soundcloudUserApi = {
  async status() {
    return await jsonFetch2(soundCloudUserApiPath("status"));
  },
  async logout() {
    await jsonFetch2(soundCloudUserApiPath("logout"), { method: "POST", body: "{}" });
  },
  async myPlaylists(limit = 20, cursor) {
    ...
    return await cachedGet2(`soundclouduser:${url}`, url);
  },
...
function bindQueueActions(opts) {
  ...
  const updateViewQueueActive = (globalTracks) => {
    ...
    const loadLikedCard = async () => {
      ...
      if (isSpotifyConnected) {
        try {
          ...
          const items = data?.items ?? [];
          const tracks = items.map((it) => it?.track).filter(Boolean).map((t) => {
            ...
            const artist = Array.isArray(t?.artists) ? t.artists.map((a) => a.name).join(", ") : "";
            const album = t?.album?.name ?? "";
            const albumId = t?.album?.id;
            const primaryArtistId = Array.isArray(t?.artists) && t.artists.length ? t.artists[0]?.id : void 0;
            return {
              id: t.id,
              source: "spotify",
              title: t.name,
      ...
      if (isSoundCloudConnected) {
        try {
          ...
          const tracks = collection.map((it) => it?.track ?? it).filter(Boolean).map((t) => {
            const id = t?.id;
            if (!id) return null;
            const title = typeof t?.title === "string" ? t.title : "(untitled)";
            const artist = typeof t?.user?.username === "string" ? t.user.username : typeof t?.user?.name === "string" ? t.user.name : "";
            const durationMs = typeof t?.duration === "number" ? t.duration : 0;
            const artUrl = typeof t?.artwork_url === "string" ? t.artwork_url : typeof t?.user?.avatar_url === "string" ? t.user.avatar_url : void 0;
            return {
              id: String(id),
              source: "soundcloud",
              title,
          ...
          for (const item of collection) {
            ...
            if (kind === "track") {
              ...
              const track = {
                ...
                title,
                artist,
                durationSec: typeof origin.duration === "number" ? Math.round(origin.duration / 1e3) : 0,
                permalinkUrl: typeof origin?.permalink_url === "string" ? origin.permalink_url : void 0,
                artUrl: typeof origin.artwork_url === "string" ? origin.artwork_url : typeof origin?.user?.avatar_url === "string" ? origin.user.avatar_url : void 0,
                artUrlSmall: typeof origin.artwork_url === "string" ? origin.artwork_url : typeof origin?.user?.avatar_url === "string" ? origin.user.avatar_url : void 0
              };
...
function mapSpotifyTrack(it) {
  ...
  const artUrl = images?.[1]?.url ?? images?.[0]?.url;
  const artUrlLarge = images?.[0]?.url ?? images?.[1]?.url ?? artUrl;
  const artist = Array.isArray(it?.artists) ? it.artists.map((a) => a.name).join(", ") : "";
  ...
  return {
    ...
    albumId: it?.album?.id,
    album,
    primaryArtistId: Array.isArray(it?.artists) && it.artists.length ? it.artists[0]?.id : void 0,
    durationSec: Math.round((it.duration_ms ?? 0) / 1e3),
    artUrl,
    artUrlSmall,
    artUrlLarge,
    uri: it.uri
...
var searchView = {
  ...
  mount(ctx) {
    const root = ctx.rootEl;
    const form = root.querySelector("[data-wa-search-form]");
    const input = root.querySelector("[data-wa-search-input]");
    const statusEl = root.querySelector("[data-wa-search-status]");
    const resultsEl = root.querySelector("[data-wa-search-results]");
    const resultsCard = root.querySelector("[data-wa-search-results-card]");
    ...
    const runSearch = async (rawQuery) => {
      ...
      try {
        ...
        if (useSpotify) {
            ...
            switch (topHit.kind) {
              ...
              case "album": {
                ...
                topHitSec.list.appendChild(createAlbumListItem({
                  album: a,
                  onClick: () => ctx.router.navigate(routePath2(`albums/${a.id}`))
                }));
                break;
              }
              case "artist": {
                ...
                topHitSec.list.appendChild(createArtistListItem({
                  artist: a,
                  onClick: () => ctx.router.navigate(routePath2(`artists/${a.id}`))
                }));
                break;
              }
              case "playlist": {
                ...
                topHitSec.list.appendChild(createPlaylistListItem({
                  playlist: p,
                  onClick: () => ctx.router.navigate(routePath2(`playlists/${p.id}`))
                }));
                break;
              }
            }
          ...
          const scTracks = trackItems.filter((it) => !!it && typeof it.id !== "undefined").map((it) => {
            const id = String(it.id);
            const title = typeof it.title === "string" ? it.title : "(untitled)";
            const artist = typeof it.user?.username === "string" ? it.user.username : typeof it.user?.name === "string" ? it.user.name : "";
            const durationMs = typeof it.duration === "number" ? it.duration : 0;
            const artUrl = typeof it.artwork_url === "string" ? it.artwork_url : typeof it.user?.avatar_url === "string" ? it.user.avatar_url : void 0;
            const permalinkUrl = typeof it?.permalink_url === "string" ? it.permalink_url : void 0;
            const track = {
              id,
              source: "soundcloud",
              title,
...
function mapSpotifyTrack2(t) {
  ...
  return {
    ...
    albumId: t?.album?.id,
    album,
    primaryArtistId: Array.isArray(t?.artists) && t.artists.length ? t.artists[0]?.id : void 0,
    durationSec: Math.round((t.duration_ms ?? 0) / 1e3),
    artUrl,
    artUrlSmall,
    artUrlLarge,
  ...
  mount(ctx) {
    ...
    const loadMoreSoundCloud = async () => {
      ...
      try {
        ...
        const collection = data?.collection ?? [];
        const next = collection.map((it) => it?.track ?? it).filter(Boolean).map((t) => {
          const id = t?.id;
          if (!id) return null;
          const title = typeof t?.title === "string" ? t.title : "(untitled)";
          const artist = typeof t?.user?.username === "string" ? t.user.username : typeof t?.user?.name === "string" ? t.user.name : "";
          const durationMs = typeof t?.duration === "number" ? t.duration : 0;
          const artUrl = typeof t?.artwork_url === "string" ? t.artwork_url : typeof t?.user?.avatar_url === "string" ? t.user.avatar_url : void 0;
          const permalinkUrl = typeof t?.permalink_url === "string" ? t.permalink_url : void 0;
          return {
            id: String(id),
            source: "soundcloud",
            title,
        ...
        try {
          ...
          const loadMoreSpotifyTracks = async () => {
            ...
            try {
              ...
              const items = data?.items ?? [];
              const next = items.map((it) => it?.track).filter(Boolean).map((t) => {
                const images = t?.album?.images ?? [];
                const artUrlSmall = images?.[images.length - 1]?.url;
                const artUrl = images?.[1]?.url ?? images?.[0]?.url;
                const artUrlLarge = images?.[0]?.url ?? images?.[1]?.url ?? artUrl;
          ...
          const loadMoreSoundCloudTracks = async () => {
            ...
            try {
              ...
              const next = items.filter((t) => !!t && typeof t.id !== "undefined").map((t) => {
                const id = t?.id;
                const title = typeof t?.title === "string" ? t.title : "(untitled)";
                const artist = typeof t?.user?.username === "string" ? t.user.username : typeof t?.user?.name === "string" ? t.user.name : "";
                const durationMs = typeof t?.duration === "number" ? t.duration : 0;
...
function loadSoundCloudWidgetApi() {
  ...
  scWidgetReadyPromise = new Promise((resolve, reject) => {
    if (w.SC && w.SC.Widget) {
      resolve();
      return;
    }
    const existing = document.querySelector("script[data-wa-sc-widget-api]");
    if (existing) {
...
function buildSoundCloudWidgetEmbedSrc(permalinkUrl) {
...
var SoundCloudTransport = class {
   ...
   */
  awaitingInitialProgress = false;
  awaitingReadySessionId = null;
  awaitingReadyResolve = null;
  ...
  getIframe() {
    ...
    let container = document.querySelector("[data-wa-sc-widget-container]");
    let iframe = null;
    if (container) {
      iframe = container.querySelector("[data-wa-sc-widget]");
    }
    if (!iframe) {
  ...
  createWidgetFromExistingIframe() {
    ...
    const widgetErrorToError = (e) => {
      try {
        if (e && typeof e === "object") {
          const msg = e.message ?? e.error ?? e.title ?? e.status;
    ...
    widget.bind(Events.SEEK, (e) => {
      if (!this.sessionStarted) return;
      const posMs = typeof e?.currentPosition === "number" ? e.currentPosition : 0;
      this.lastPositionSec = Math.max(0, posMs / 1e3);
    });
    widget.bind(Events.FINISH, () => {
      if (!this.currentTrack) return;
      this.lastKnownPlaying = false;
      const finishedId = this.currentTrack.id;
      const duration = typeof this.currentTrack.durationSec === "number" && Number.isFinite(this.currentTrack.durationSec) ? this.currentTrack.durationSec : this.lastPositionSec;
      this.emitRemote({
        track: this.currentTrack,
        isPlaying: false,
  ...
  async callWidgetGetter(invoke, timeoutMs, fallback) {
    return await new Promise((resolve, reject) => {
      let done = false;
      const t = window.setTimeout(() => {
        if (done) return;
        done = true;
        if (arguments.length >= 3) resolve(fallback);
  ...
  async getPositionMs(widget, timeoutMs = 800) {
    const ms = await this.callWidgetGetter((cb) => widget.getPosition(cb), timeoutMs, 0);
    return typeof ms === "number" && Number.isFinite(ms) ? ms : 0;
  }
  async getDurationMs(widget, timeoutMs = 800) {
    const ms = await this.callWidgetGetter((cb) => widget.getDuration(cb), timeoutMs, 0);
    return typeof ms === "number" && Number.isFinite(ms) ? ms : 0;
  }
  async getCurrentSound(widget, timeoutMs = 800) {
    return await this.callWidgetGetter((cb) => widget.getCurrentSound(cb), timeoutMs, null);
  }
  async waitForCurrentSoundId(widget, expectedTrackId, timeoutMs = 1600) {
    const expected = String(expectedTrackId);
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      try {
        const s = await this.getCurrentSound(widget, 650);
        const id = s && (s.id ?? s.soundId ?? s.sound_id);
  ...
  async getIsPaused(widget) {
    return await this.callWidgetGetter((cb) => widget.isPaused(cb), 800, true);
  }
  async emitActualPlaybackState(widget) {
    if (!this.currentTrack) return;
    try {
      const ok = await this.waitForCurrentSoundId(widget, this.currentTrack.id, 650);
      if (ok) {
        const posMs = await this.getPositionMs(widget, 500);
        const posSec = Math.max(0, posMs / 1e3);
  ...
  async play(track, positionSec = 0, opts) {
    ...
    try {
      ...
      const trackUrl = typeof track?.permalinkUrl === "string" && track.permalinkUrl.trim().length ? track.permalinkUrl.trim() : `https://api.soundcloud.com/tracks/${encodeURIComponent(track.id)}`;
      if (autoplay) {
        try {
      ...
      const readyAfterLoad = new Promise((resolve) => {
        this.awaitingReadySessionId = sessionAtStart;
        this.awaitingReadyResolve = resolve;
      });
      const desiredPosMs = Math.max(0, (this.lastPositionSec || 0) * 1e3);
      const loadCallback = new Promise((resolve, reject) => {
        try {
...
function boot() {
  ...
  window.addEventListener("wa:track:select", (e) => {
    ...
    if (Array.isArray(ev.detail?.tracks)) {
      playerStore.setQueue(ev.detail.tracks, { wrap: ev.detail?.wrap ?? false });
    }
    playerStore.selectTrackById(trackId, true);
  });
